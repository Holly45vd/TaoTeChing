...
export default function StoryDrawer({
  ...
  const handleClipStory = useCallback(async () => {
    ...
    try {
      ...
      setSnack({ open: true, msg: "저장에 실패했어. (권한/네트워크 확인)" });
    }
  }, [canSave, storyText, uid, chapterNumber, chapterTitle, story?.title]);
        ...
        <Stack sx={{ height: "100%" }}>
          ...
          <Box sx={{ p: 2, pb: 1.5 }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between">
              ...
              <IconButton onClick={onClose} size="small" aria-label="닫기" title="닫기">
          ...
          <Box sx={{ flex: 1, overflowY: "auto", px: 2, pb: 2 }}>
            {loading ? (
              ...
              <Alert severity="info">
                아직 이 장의 동화가 없어. <br />
                (Firestore에 {String(chapterNumber)} 문서를 만들면 바로 뜬다)
              </Alert>
            ) : (
              <Box>
                {!!story?.title && (
